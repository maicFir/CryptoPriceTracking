...
const isOtherWebsite = (url: string) => {
  const isHttpPre = /^(http|https)/g.test(url);
  if (!isHttpPre) {
    return false;
  }
  const whiteIp = ["https://www.binance.com", "https://api.binance.com"];
  const { origin } = new URL(url);
  return whiteIp.some((v) => origin === v);
};
// 请求拦截器
  ...
  (config: AxiosRequestConfig & any) => {
    ...
    if (IsNotWebSite(config.url)) {
      if (config.headers) {
        ...
        const { access_token: authToken } = storageUtils.getStorage(
          STORAGE_NAME.USER_INFO
        ) || { access_token: "" };
...
service.interceptors.response.use(
  (response: any) => {
    ...
    if (response.status !== 200) {
      ...
      if (isOtherWebsite((response.config as any).url)) {
      ...
      if (code === 200) {
        ...
        if (!response.config.headers.message) {
          if (code === 403) {
            ...
            // 清除所有缓存
            storageUtils.clearSomeKey("rulesShow");
          } else {
            // window.alert(msg);
            Message.alert({
              msg,
